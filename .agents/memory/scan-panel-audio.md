...
**Fix:** attach a continuously-running oscillator at −100 dB gain on `startAudioKeepalive()`. While any audio node is actively outputting, iOS cannot suspend the context. Schedule melody notes with the same synchronous `getCtx()` path used by every other working function — do NOT use an async Promise chain (`getRunningCtx().then()`); the async path silently swallows errors and the melody is never scheduled. Triangle-wave oscillators at low gain (0.15) are inaudible on phone speakers — use `"square"` type at 0.17–0.19 gain to match the ok/err tones that reliably work.
