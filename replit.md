...
- **Fulfillment:** Order fulfillment system, grouping items by warehouse bin location. Features a consolidated Actions dropdown menu for print/ship operations including: packing slips (multi-select), lot labels (placeholder for future), and shipping (single order). Packing slip system supports bulk print with PlanetBrick branding and order details.
