...
- **Order Status Mapping:** Centralized configuration (`server/config/order-status-mapping.ts`) maps platform-specific statuses to normalized statuses and defines inventory impact. Critical logic: inventory is reduced ONLY when orders ship (any status → `shipped`), and restored ONLY when shipped orders are cancelled/returned (`shipped` → `cancelled`/`returned`). Orders cancelled before shipping do NOT adjust inventory (prevents incorrect inventory increases). Supports BrickLink, BrickOwl, ShipStation, eBay, and Amazon platforms.
