...
- **Order Status Mapping:** Centralized configuration (`server/config/order-status-mapping.ts`) maps platform-specific statuses to normalized statuses and defines inventory impact. Status transitions trigger inventory adjustments: `reduce` (order placed/on hold), `restore` (cancelled/returned), `none` (no change). Supports BrickLink, BrickOwl, ShipStation, eBay, and Amazon platforms.
