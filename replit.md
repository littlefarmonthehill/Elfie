...
-   **Customer Shopping Platform (Oct 2025):** Mobile-first storefront at `/shop` featuring **part-number-based product grouping** (Oct 22, 2025). Products are grouped by unique part number (itemNo), with all color/condition variations aggregated into a single product. Each product card displays accurate counts: unique colors, New variations, and Used variations, with total quantity summed across all variations. Card width increased to w-52/w-80 (from w-44/w-72) to prevent text wrapping of badges showing color count, New quantity, and Used quantity. Scoring formula for product ranking: `(total_value * 0.7) + (log10(total_qty + 1) * 0.3)` where total_value is the sum of (price × quantity) for all variations. **Redesigned Header (Oct 21, 2025):** Stark black background with centered PlanetBrick logo (half in/half out of banner), cart and login buttons in top-right corner. Stats (Unique Lots/Total Parts) positioned below logo in center with animated "comet trail" effects on hover. Banner height reduced by 1/3. Sticky search bar positioned below header. Category sections use blue/cyan/gray/white gradient color schemes with minimal red accents for better visual balance. Product detail modal redesigned: compact layout with larger text for easier reading, no autofocus on inputs to prevent mobile keyboard popup, fluid animations, gradient buttons. Product grid shows 2 columns on mobile scaling to 6 on desktop. Each product card displays color count badge, emerald-colored "New" badge (if applicable), amber-colored "Used" badge (if applicable), and total quantity in cyan. Clicking opens sleek modal showing all color/condition variations grouped by color with interactive quantity controls. Jetsons retro-futuristic aesthetic: pure black background with popping blue/cyan/white gradients, animated orbital rings, floating color orbs, glow effects. Strategic alternative to Shopify providing full BrickLink/BrickOwl integration control and cost savings.
