...
-   **SKU Standardization (Oct 23, 2025):** The `order_details.sku` field is standardized to always contain the BrickLink inventory ID for cross-platform data integrity. BrickLink orders populate SKU from `inventory_id`, BrickOwl orders populate SKU from `external_lot_ids.other`. This enables consistent joins to `bl_inventory.id` for analytics, category analysis, and AI tools. All queries updated to join on `sku = CAST(bl_inventory.id AS TEXT)`.
