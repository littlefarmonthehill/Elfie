...
-   **SKU Standardization (Oct 23, 2025):** The `order_details.sku` field is standardized to always contain the BrickLink inventory ID for cross-platform data integrity. BrickLink orders populate SKU from `inventory_id`, BrickOwl orders populate SKU from `external_lot_ids.other`. **Automatic SKU Migration:** BrickOwl order sync now includes intelligent migration logic that detects historical orders with BOID-format SKUs (BrickOwl IDs) and automatically updates them to BrickLink inventory IDs using `external_lot_ids.other` from the API response. This ensures gradual migration of all historical BrickOwl order data during regular sync operations. All queries updated to join on `sku = CAST(bl_inventory.id AS TEXT)`.
