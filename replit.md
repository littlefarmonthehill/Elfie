...
- **Order Sync Tester:** Dry-run testing tool for validating order sync mapping logic from BrickLink and BrickOwl APIs to the existing database schema. Located in Orders Dashboard Tools section. Allows testing with 3-20 historical orders from either platform without writing to the database. Displays comprehensive comparison report showing: platform badges, order numbers, customer info, ShipStation comparison status (exists or not), item-level SKU mapping, warehouse bin locations, and detected issues (e.g., missing inventory IDs). Uses BrickLink inventory ID as SKU for order items (from `inventory_id` in BrickLink, `external_lot_ids.other` in BrickOwl). Backend services (`bricklink-orders.ts`, `brickowl-orders.ts`) fetch and map order data via POST `/api/orders/dry-run-test` endpoint. Critical for Phase 1 UAT validation before cutover from ShipStation to direct platform APIs.
