...
- **Fulfillment (Warehouse Missions):** Order fulfillment system located in the Default Dashboard's "Missions" section. Displays orders awaiting fulfillment (statuses: awaiting_payment, awaiting_fulfillment, awaiting_shipment) in a 4-column grid with alphanumeric sorting. Items are grouped by warehouse bin location, showing aisle › shelf › bin hierarchy. Each item has a fulfill checkbox to track individual line item completion. Badge indicator shows count of orders requiring fulfillment. Items persist in the list until the parent order's ShipStation status changes. Uses `selectDistinct` to prevent duplicate rows from multiple inventory records with the same SKU. Fulfillment status tracked via `fulfilled` and `fulfilledAt` fields in the `order_details` table.
