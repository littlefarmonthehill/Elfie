...
- **Order Status Mapping:** Centralized configuration (`server/config/order-status-mapping.ts`) maps platform-specific statuses to normalized statuses and defines inventory impact. Status transitions trigger inventory adjustments: `reduce` (shipped only), `restore` (cancelled/returned), `none` (all other statuses). Inventory is reduced when orders ship, restored when cancelled/returned. Supports BrickLink, BrickOwl, ShipStation, eBay, and Amazon platforms.
