...
- **Dashboard Layout:** Features a default dashboard with a "Missions" section for actionable tasks (picklist), with dedicated Inventory, Orders, Sales, and Marketing dashboards. Orders and Marketing dashboards include date range selectors. The Sales Dashboard offers Year-over-Year and Platform Comparison features. The Inventory Dashboard provides drawer-based tool access for Price-o-Matic, Warehouse Management, and Platform Sync.
- **Picklist (Warehouse Missions):** Bin-level picking system for order fulfillment located in the Default Dashboard's "Missions" section. Groups picklist items by warehouse bins with pull/reshelve tracking. Shows pending bin counts as indicator badges. Bins are organized by Aisle (desc) → Shelf (asc) → Bin (asc) for optimal picking routes. Supports bins without warehouse locations (displayed as "Unassigned"). Features expandable bin details to show individual items, orders, and quantities.
