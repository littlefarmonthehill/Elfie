...
**eBay Channel Integration (Beta):** `server/services/ebay.ts` implements BrickLink→eBay synchronization using the eBay Sell Inventory API. Key design decisions: (1) SKU format `BL-{blInventoryId}` — the SKU IS the BrickLink lot ID. (2) OAuth 2.0 with refresh token stored in `org_integrations` (channel='ebay'); access tokens cached in-memory with expiry tracking. (3) Images resolved from Image Center (lot-level → item-type-level → catalog fallback); served via `/api/user-images/:key` proxy — NEVER from BrickLink CDN. (4) For LEGO Sets, eBay catalog matching is attempted via structured item specifics (Set Number, Brand=LEGO, Type=Complete Set). (5) `channelSyncConfig.channelConfig` JSONB stores eBay-specific settings: `ebayBlIdField` (custom_label | item_specifics), `ebayCatalogMatch`, `ebayListingDuration` (GTC), `syncImages`, `ebayMarketplaceId`, `ebayConditionUsed`. (6) When an eBay integration is created via POST `/api/org/integrations`, a default `channelSyncConfig` row is auto-created so the scheduler picks it up. Credentials configured in Settings → Platform Connections → eBay. eBay appears in the Channel Sync panel as a second tab when configured; the sync always runs as a push operation. **eBay Order Ingest:** `server/services/ebay-order-sync.ts` implements inbound order sync via the eBay Sell Fulfillment API. Uses `lastmodifieddate:[{ISO}..]` filter for incremental sync (30-min lookback buffer, same as BrickOwl). Status mapping: `cancelState=CANCELED`→cancelled, `FULFILLED`→completed, `NOT_STARTED|IN_PROGRESS`→paid. Order ID format: `ebay-{ebayOrderId}`. Sync metadata key: `ebay_orders`. Manual trigger: POST `/api/sync/ebay/orders`. Scheduled automatically — see Universal Channel Order Sync below.
