...
- **Service Worker & PWA:** Intelligent update system that checks for new versions only when the app is opened (no background polling), with smart auto-reload that waits for safe moments (no open drawers/modals, page visible) before applying updates to prevent interruptions during critical user interactions like camera usage or settings changes.
