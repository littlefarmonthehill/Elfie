...
- **Order Status Mapping & Inventory Automation:** Centralized configuration (`server/config/order-status-mapping.ts`) maps platform-specific statuses to normalized statuses and defines inventory impact. **IMPLEMENTED** automatic inventory adjustment system (`server/services/inventory-adjustment.ts`) with critical logic: inventory is reduced ONLY when orders ship (any status → `shipped`), and restored ONLY when shipped orders are cancelled/returned (`shipped` → `cancelled`/`returned`). Orders cancelled before shipping do NOT adjust inventory (prevents incorrect inventory increases). Tracks previous order status for proper transition logic. Integrated with ShipStation sync - inventory adjustments trigger automatically during order sync when status changes. API endpoints: `POST /api/orders/:id/status` (update status + adjust inventory), `POST /api/orders/:id/adjust-inventory` (manual adjustment). Uses `GREATEST(0, ...)` to prevent negative stock, collects per-line errors for missing BrickLink inventory IDs. Supports BrickLink, BrickOwl, ShipStation, eBay, and Amazon platforms.
...
- **Order Sync Tester:** Dry-run testing tool for validating order sync mapping logic from BrickLink and BrickOwl APIs to the existing database schema. Located in Orders Dashboard Tools section. Allows testing with 3-20 orders from either platform without writing to the database. Features collapsible Accordion sections showing side-by-side comparison between ShipStation and platform data: Order Header section displays 4-column table (Field | ShipStation | Field | Platform) comparing order number, customer, email, order date, status, and total; Order Detail section shows shipping address fields and line items with SKU mapping and warehouse bin locations. Gracefully handles missing credentials by skipping that platform instead of blocking results. Uses BrickLink inventory ID as SKU for order items (from `inventory_id` in BrickLink, `external_lot_ids.other` in BrickOwl). Backend services (`bricklink-orders.ts`, `brickowl-orders.ts`) fetch and map order data via POST `/api/orders/dry-run-test` endpoint. **IMPORTANT LIMITATION:** BrickLink and BrickOwl APIs do not return order items for shipped/completed orders (APIs purge detailed item data post-fulfillment). Tester fetches pending orders (`awaiting_payment`, `awaiting_fulfillment`, `awaiting_shipment` statuses) from ShipStation to ensure items are available from platform APIs. For UAT validation, test with NEW/PENDING orders before Phase 1 cutover from ShipStation to direct platform APIs.
