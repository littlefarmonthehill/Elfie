...
- **Year-over-Year Sales Comparison (October 2025):** Sales Dashboard features a comprehensive YoY comparison tool that allows users to compare current year revenue against any two previous years. Toggle-based UI with year selectors provides flexibility to analyze growth patterns and identify anomalous years. Multi-line chart displays month-by-month revenue for all three years with color-coded lines (green for current year, blue/purple for comparison years). Growth metrics section shows percentage changes and revenue totals. Feature respects all date range filters (MTD, 3M, 6M, 1Y, 2Y, All), applying same time windows across all comparison years for accurate YoY analysis. Chart dynamically adjusts X-axis to show only relevant months within selected range, avoiding misleading zero-value displays. Implementation uses `monthsToShow - 1` calculation to ensure exact month counts (e.g., 3M = current month + 2 prior months = 3 total). Production-ready after comprehensive testing and architect review (Oct 13, 2025).
