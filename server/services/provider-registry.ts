...
export const PROVIDER_REGISTRY: Record<string, ProviderDefinition> = {
  ...
  ebay: {
    ...
    type:        'sales_channel',
    status:      'beta',
    description: 'Push your BrickLink inventory to eBay fixed-price listings. Uses your Image Center photos and eBay catalog data for LEGO sets.',
    accentColor: 'yellow',
    docsUrl:     'https://developer.ebay.com/develop/guides/sell-api-overview',
    credentialFields: [
      { key: 'appId',        label: 'App ID (Client ID)',       type: 'text',     required: true,  placeholder: 'MyApp-…',      helpText: 'Found in your eBay Developer account → Application Keys.' },
      { key: 'certId',       label: 'Cert ID (Client Secret)',  type: 'password', required: true,  placeholder: 'SBX-…',        helpText: 'Client Secret from the same Application Keys page.' },
      { key: 'devId',        label: 'Dev ID',                   type: 'text',     required: false, placeholder: 'xxxxxxxx-…',   helpText: 'Optional — only needed for Trading API (not Inventory API).' },
      { key: 'refreshToken', label: 'OAuth Refresh Token',      type: 'password', required: true,  placeholder: 'v^1.1…',       helpText: 'Obtained by authorizing your eBay seller account. Use the Connect button to get this automatically.' },
      {
        key:      'environment',
        label:    'Environment',
        type:     'select',
