...
// In production, serveStatic is added here so the SPA is available immediately.
// In development, setupVite runs after registerRoutes (it needs the server handle).
...
httpServer.listen({ port, host: "0.0.0.0" }, () => {
  ...
  (async () => {
    try {
      ...
      // 4e. Global error handler (must be after routes)
      ...
      // 4f. In development, wire up Vite HMR (passes the real httpServer for WebSocket)
      if (!isProduction) {
