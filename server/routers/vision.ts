...
async function processBrickanalyzerScan(scanId: number, imageBuffer: Buffer, settings: Record<string, number> = {}, calibration = false, previewBoxes?: { x: number; y: number; w: number; h: number }[], orgId: string = 'org_planetbrick') {
  ...
  try {
    ...
    if (previewBoxes && previewBoxes.length > 0) {
      ...
      const pass1: Record<string, any> = { segmenter: 'blob', normalizeBackground: true, minSizePct: 0.40, maxSizePct: 55, maxDimFrac: 90, blurRadius: 5, closeK: 11, closeIter: 2, satThresh: 40 };
...
router.post("/brickanalyzer/segment", brickanalyzerUpload.single('image'), isApproved, async (req, res) => {
  try {
    ...
    const blobPass: Record<string, any> = { segmenter: 'blob', normalizeBackground: true, minSizePct: 0.40, maxSizePct: 55, maxDimFrac: 90, blurRadius: 5, closeK: 11, closeIter: 2, satThresh: 40 };
