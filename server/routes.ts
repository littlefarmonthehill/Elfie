...
<body>
  ...
  app.post('/api/platform-admin/platform-services/ebay-credentials', isSuperAdmin, async (req, res) => {
    try {
      ...
      // Map of request body key → SQL column name (used for raw parameterized UPDATE)
      const fieldMap: Array<[string, string]> = [
        ['prodAppId',         'ebay_prod_app_id'],
        ['prodCertId',        'ebay_prod_cert_id'],
        ['prodDevId',         'ebay_prod_dev_id'],
        ['prodRuName',        'ebay_prod_ru_name'],
        ['sandboxAppId',      'ebay_sandbox_app_id'],
        ['sandboxCertId',     'ebay_sandbox_cert_id'],
        ['sandboxDevId',      'ebay_sandbox_dev_id'],
        ['sandboxRuName',     'ebay_sandbox_ru_name'],
      ...
      // Build parameterized raw SQL — guaranteed to write exactly what we pass
      ...
      await db.execute(
        sql.raw('UPDATE platform_settings SET ').append(setFragment).append(sql.raw(", updated_at = NOW() WHERE id = 'platform'"))
      );
...
Analyze this store's inventory and respond with ONLY a valid JSON object (no markdown, no extra text) with this exact structure:
{
  ...
  // Redirects the user's browser to eBay's OAuth authorization page.
  app.get('/api/ebay/oauth/initiate', isApproved, async (req: any, res) => {
    try {
      ...
      res.status(500).json({ message: err?.message || 'Failed to initiate eBay OAuth' });
    }
  });

  // GET /api/ebay/oauth/callback — eBay redirects here after the user authorizes.
  // No auth middleware — eBay calls this directly as a redirect target.
  ...
  // GET /api/ebay/connection/status — current org's eBay connection state
  app.get('/api/ebay/connection/status', isApproved, async (req: any, res) => {
    try {
      const orgId = reqOrgId(req);
  ...
  // DELETE /api/ebay/connection?env=production|sandbox — revoke a token
  app.delete('/api/ebay/connection', isApproved, async (req: any, res) => {
    try {
      const orgId = reqOrgId(req);
      const env = (req.query.env as string) === 'sandbox' ? 'sandbox' : 'production';
      const tokenKey = env === 'sandbox' ? 'sandboxRefreshToken' : 'refreshToken';
  ...
  // POST /api/ebay/connection/environment — switch active environment (prod/sandbox)
  app.post('/api/ebay/connection/environment', isApproved, async (req: any, res) => {
    try {
      const orgId = reqOrgId(req);
