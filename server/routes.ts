...
<body>
  ...
  // POST /api/platform-admin/cleanup-duplicate-bo-orders
  // Finds bo- prefix orders that are duplicates of existing numeric-ID orders,
  // restores the inventory that was incorrectly deducted, and deletes the duplicates.
  // ?confirm=true to execute; default is dry run.
  app.post('/api/platform-admin/cleanup-duplicate-bo-orders', isSuperAdmin, async (req, res) => {
    try {
      const confirm = req.query.confirm === 'true';

      // Identify duplicate bo- orders: a bo- order whose order_number matches an existing non-bo- order
      const dupResult = await db.execute(sql`
        SELECT bo.id AS dup_id
        FROM orders bo
      ...
      `);
      const dupIds = (dupResult.rows as any[]).map(r => r.dup_id as string);
      ...
      if (!confirm) {
        ...
        `);
        const inventoryItemsResult = await db.execute(sql`
          SELECT COUNT(*) as cnt
          FROM order_details od
          WHERE od.order_id IN (
            SELECT bo.id FROM orders bo
          ...
          AND od.bricklink_inventory_id IS NOT NULL
        `);
        return res.json({
          dryRun: true,
          duplicateOrdersToDelete: dupIds.length,
          orderDetailsToDelete: Number((detailCountResult.rows[0] as any)?.cnt ?? 0),
          inventoryItemsToRestore: Number((inventoryItemsResult.rows[0] as any)?.cnt ?? 0),
          sampleIds: dupIds.slice(0, 10),
          message: 'Pass ?confirm=true to execute the cleanup and inventory restoration',
        });
      }

      if (dupIds.length === 0) {
        return res.json({ message: 'No duplicate bo- orders found — nothing to clean up.' });
      }

      // Step 1: Restore bl_inventory quantities for items in the duplicate orders
      ...
      // Step 2: Delete order_details for the duplicate orders
      await db.execute(sql`
        DELETE FROM order_details
        WHERE order_id IN (
          SELECT bo.id FROM orders bo
      ...
      console.log(`[AdminCleanup] Deleted ${dupIds.length} duplicate bo- BrickOwl orders and restored their inventory`);

      return res.json({
        success: true,
        ordersDeleted: dupIds.length,
        inventoryRowsRestored: (restoreResult as any).rowCount ?? 0,
        message: `Deleted ${dupIds.length} duplicate bo- BrickOwl orders and restored inventory quantities`,
      });
    } catch (error: any) {
      console.error('[AdminCleanup] Error during bo- duplicate order cleanup:', error.message);
      res.status(500).json({ error: 'Cleanup failed: ' + error.message });
    }
  });
    ...
    try {
      ...
      if (existing) {
        ...
        const [inserted] = await db.insert(channelSyncConfig).values({
          ...
          syncTierPrice:    (patch.syncTierPrice      as boolean)  ?? true,
...
Be specific with numbers. Reference actual data points. Return ONLY a JSON array, no markdown, no explanation.`;
  ...
  app.post("/api/channel-sync/test-price-update", isApproved, async (req: any, res) => {
    try {
...
Respond ONLY as JSON: {"price": 0.00, "reasoning": "..."}`;
  ...
  // Server-side search across ALL inventory (bypasses the 2000-item pre-load limit)
  app.get("/api/warehouse/inventory/search", isApproved, async (req: any, res) => {
    try {
      const orgId = reqOrgId(req);
