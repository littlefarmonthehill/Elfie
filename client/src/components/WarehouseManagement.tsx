...
export default function WarehouseManagement({ onItemClick }: WarehouseManagementProps) {
  ...
  const assignInventoryMutation = useMutation({
    ...
    onSuccess: () => {
      // Invalidate all affected queries: unassigned items, bins (item counts), shelves, aisles, locations
          ...
          <Card className="p-3">
            ...
            <div className="space-y-2 text-xs">
              <div className="flex justify-between">
                <span className="text-gray-400">Unassigned Items</span>
    ...
    return (
      <Card className="p-4">
        <div className="text-center text-sm text-gray-400">
          All inventory items are assigned to bins
        </div>
      </Card>
    );
  }
  ...
  return (
    <Card className="p-3">
      <div className="flex items-center gap-2 mb-3">
        ...
        <h3 className="text-sm font-semibold">Unassigned Inventory ({items.length})</h3>
      </div>
...
function StructureView({ aisles, shelves, bins }: any) {
      ...
      {aisles.map((aisle: any) => {
        ...
        return (
          <Card key={aisle.id} className="p-2">
            <div
              className="flex items-center justify-between cursor-pointer hover-elevate rounded p-2"
                      ...
                      {isShelfExpanded && (
                        <div className="ml-5 mt-1 space-y-1">
                          {shelfBins.map((bin: any) => (
                            <div key={bin.id} className="flex items-center gap-2 hover-elevate rounded p-1.5 text-xs" data-testid={`bin-${bin.id}`}>
                              <Package className="w-3 h-3 text-orange-400" />
                              <span className="text-gray-400">{bin.name}</span>
                              <Badge variant="outline" className="text-[9px]">{bin.itemCount} items</Badge>
                            </div>
                          ))}
                        </div>
                      )}
