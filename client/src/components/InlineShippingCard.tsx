...
export default function InlineShippingCard({
  ...
  // Renders a string highlighting any non-ASCII characters in red, for the address display.
  const highlightNonAscii = (text: string) => {
    ...
    return (
      <>
        {parts.map((part, i) =>
          /[^\x00-\x7F]/.test(part)
            ? <span key={i} className="text-red-400 font-bold bg-red-950/40 rounded px-0.5">{part}</span>
            : part
        )}
      </>
    );
  ...
  // Detect non-ASCII characters in the destination address.
  // USPS / EasyPost label printers are ASCII-only — non-Latin characters (Japanese,
  // Chinese, Arabic, etc.) silently become '?' on the printed label.
  const hasNonAscii = /[^\x00-\x7F]/.test(
    [currentAddress?.name, currentAddress?.street1, currentAddress?.street2, currentAddress?.city]
      .filter(Boolean).join(' ')
  );
  ...
  return (
    ...
    >
      <div className="px-3 pt-2.5 pb-2 space-y-2">
        ...
        {/* ── Non-ASCII address warning ── */}
        {hasNonAscii && (
          <div className="rounded-md border border-red-500/50 bg-red-950/25 px-2.5 py-2 space-y-1" data-testid="banner-non-ascii-address">
            <div className="flex items-center gap-1.5">
              <AlertTriangle className="w-3.5 h-3.5 shrink-0 text-red-400" />
              <span className="text-[11px] font-semibold text-red-300">Non-Latin characters detected in address</span>
            </div>
            <p className="text-xs text-gray-300 leading-snug">
              USPS labels only support ASCII text. Characters like Japanese, Chinese, Arabic, or Korean will print as <span className="font-mono text-red-400 font-bold">?</span> on the label, which can cause delivery failure.
            </p>
            <p className="text-xs text-amber-300 leading-snug">
              Open <strong>Show details → Edit address</strong> and enter a romanized (English) version of the name and address before buying this label.
            </p>
          </div>
        )}
          ...
          <div className="px-3 py-2.5 space-y-1.5">
            ...
            {!editingAddress ? (
              <div className="text-xs text-gray-300 leading-snug">
                {currentAddress?.name && <div className="font-medium">{highlightNonAscii(currentAddress.name)}</div>}
                {currentAddress?.street1 && <div>{highlightNonAscii(currentAddress.street1)}</div>}
                {currentAddress?.street2 && <div>{highlightNonAscii(currentAddress.street2)}</div>}
                <div>
                  {currentAddress?.city && highlightNonAscii(currentAddress.city)}
                  {[currentAddress?.state, currentAddress?.zip].filter(Boolean).length > 0 &&
