    ...
    <div className="p-2 space-y-1.5 bg-gradient-to-br from-lego-yellow/5 to-transparent rounded-lg border border-lego-yellow/10 shadow-[0_0_15px_rgba(234,179,8,0.1)]">
      ...
      <div className="bg-gray-900/50 border border-yellow-500/20 rounded-lg p-4 md:p-5" data-testid="section-customer-metrics">
        <div className="flex items-center gap-2 mb-3">
          ...
          <h3 className="text-xs md:text-base lg:text-lg font-semibold text-yellow-400 uppercase tracking-wide">Customer Metrics</h3>
        </div>
        <div className="grid grid-cols-3 gap-2 md:gap-4">
          <div className="text-center">
            <div className="text-[11px] md:text-sm lg:text-base text-gray-400 mb-1">Total Customers</div>
            <div className="text-lg md:text-xl text-gray-200 font-mono font-semibold">{totalCustomers}</div>
          </div>
          <div className="text-center">
            <div className="text-[11px] md:text-sm lg:text-base text-gray-400 mb-1">Repeat Rate</div>
            <div className="text-lg md:text-xl text-lego-green font-mono font-semibold">{repeatCustomerRate}%</div>
          </div>
          <div className="text-center">
            <div className="text-[11px] md:text-sm lg:text-base text-gray-400 mb-1 whitespace-nowrap">Avg Orders/Customer</div>
      ...
      <div className="bg-gray-900/50 border border-green-500/20 rounded-lg p-3 md:p-4" data-testid="section-top-customers">
        <div className="flex items-center gap-2 mb-2">
          <Star className="w-3.5 h-3.5 md:w-5 md:h-5 lg:w-6 lg:h-6 text-green-400" />
          <h3 className="text-xs md:text-base lg:text-lg font-semibold text-green-400 uppercase tracking-wide">Highlights - Top Customers by Revenue</h3>
        </div>
            ...
            topCustomers.map((customer, idx) => (
              ...
              >
                <div className="flex items-center gap-2 flex-1 min-w-0">
                  <Star className="w-3.5 h-3.5 md:w-5 md:h-5 lg:w-6 lg:h-6 text-green-400 flex-shrink-0" />
                  <span className="text-gray-200 text-xs md:text-base lg:text-lg font-medium">{customer.customerUsername}</span>
                  <span className="text-gray-400 text-[11px] md:text-sm lg:text-base">{customer.orderCount} {customer.orderCount === 1 ? 'order' : 'orders'}</span>
                </div>
                <span className="text-lego-green font-mono font-medium text-xs md:text-base lg:text-lg ml-2 flex-shrink-0">${customer.totalRevenue.toFixed(2)}</span>
              </div>
            ))
          ) : (
      ...
      <div className="bg-gray-900/50 border border-blue-500/20 rounded-lg p-3 md:p-4" data-testid="section-repeat-customers">
        <div className="flex items-center gap-2 mb-2">
          <TrendingUp className="w-3.5 h-3.5 md:w-5 md:h-5 lg:w-6 lg:h-6 text-blue-400" />
          <h3 className="text-xs md:text-base lg:text-lg font-semibold text-blue-400 uppercase tracking-wide">Action Items - Engage Repeat Customers</h3>
        </div>
        <div className="space-y-1.5">
          {repeatCustomers.length > 0 ? (
            repeatCustomers.map((customer, idx) => (
              ...
              >
                <div className="flex items-center gap-2 flex-1 min-w-0">
                  <Users className="w-3.5 h-3.5 md:w-5 md:h-5 lg:w-6 lg:h-6 text-blue-400 flex-shrink-0" />
                  <span className="text-gray-200 text-xs md:text-base lg:text-lg font-medium">{customer.customerUsername}</span>
                  <span className="text-gray-400 text-[11px] md:text-sm lg:text-base">{customer.orderCount}x buyer</span>
                </div>
                <span className="text-lego-green font-mono font-medium text-xs md:text-base lg:text-lg ml-2 flex-shrink-0">${customer.totalRevenue.toFixed(2)}</span>
              </div>
            ))
      ...
      <div className="bg-gray-900/50 border border-purple-500/20 rounded-lg p-3 md:p-4" data-testid="section-new-customers">
        <div className="flex items-center gap-2 mb-2">
          ...
          <h3 className="text-xs md:text-base lg:text-lg font-semibold text-purple-400 uppercase tracking-wide">Recent Activity - New Customers (Last 30 Days)</h3>
        </div>
        <div className="space-y-1.5">
            ...
            recentNewCustomers.map((customer, idx) => (
              ...
              >
                <div className="flex items-center gap-2 flex-1 min-w-0">
                  ...
                  <span className="text-gray-200 text-xs md:text-base lg:text-lg font-medium">{customer.customerUsername}</span>
                  <span className="text-gray-400 text-[11px] md:text-sm lg:text-base">{new Date(customer.lastOrderDate).toLocaleDateString()}</span>
                </div>
                <span className="text-lego-green font-mono font-medium text-xs md:text-base lg:text-lg ml-2 flex-shrink-0">${customer.totalRevenue.toFixed(2)}</span>
              </div>
            ))
