...
const BrickanalyzerTool = forwardRef((_, ref) => {
  ...
  return (
    <div className="space-y-4 sm:space-y-5 p-1 sm:p-3">

      {/* ── BrickLink API limit warning ──────────────────────────────────── */}
      {blRateLimit?.blocked && (
        <div className="flex items-start gap-2.5 bg-red-950/50 border border-red-500/40 rounded-lg px-3 sm:px-5 py-2.5 sm:py-3.5">
          <AlertTriangle className="w-4 h-4 sm:w-5 sm:h-5 text-red-400 shrink-0 mt-0.5" />
          <div className="space-y-0.5 min-w-0">
            <p className="text-xs sm:text-sm font-semibold text-red-300">BrickLink API limit reached</p>
      ...
      {uiState === "idle" && (
        <>
          ...
          />
          <button
            className="w-full flex flex-col items-center justify-center gap-3 py-16 sm:py-28 text-center rounded-xl border border-dashed border-gray-700 hover:border-purple-500/50 hover:bg-purple-950/20 transition-colors cursor-pointer"
            data-testid="button-brickanalyzer-idle-trigger"
            onClick={() => fileInputRef.current?.click()}
          >
            <Camera className="w-12 h-12 sm:w-20 sm:h-20 text-gray-500" />
          ...
          <div className="rounded-lg border border-gray-700 bg-gray-900/40 overflow-hidden">
            ...
            <div className="flex">
              <button
                className={`flex-1 py-2.5 sm:py-3.5 text-xs sm:text-sm font-semibold transition-colors ${scanMode === "auto" ? "bg-purple-600 text-white" : "text-gray-400 hover:text-gray-300 hover:bg-gray-800/60"}`}
              ...
              <button
                className={`flex-1 py-2.5 sm:py-3.5 text-xs sm:text-sm font-semibold transition-colors border-l border-gray-700 ${scanMode === "manual" ? "bg-purple-600 text-white" : "text-gray-400 hover:text-gray-300 hover:bg-gray-800/60"}`}
            ...
            {scanMode === "auto" && (
              <div className="px-3 py-2.5 sm:px-5 sm:py-4 border-t border-gray-700/60 space-y-1">
                <p className="text-[11px] sm:text-sm text-gray-300 font-medium">Smart 3-pass scan</p>
            ...
            >
              <span className="flex items-center gap-2 text-xs sm:text-sm font-medium text-gray-400">
            ...
            {showSettings && (
              <div className="px-3 sm:px-5 pb-3 sm:pb-5 space-y-4 sm:space-y-5 border-t border-gray-700 pt-3 sm:pt-5">
      ...
      {/* ── UPLOADING ───────────────────────────────────────────────────── */}
      {uiState === "uploading" && (
        <div className="flex flex-col items-center gap-3 py-12 sm:py-20">
          <Loader2 className="w-8 h-8 sm:w-14 sm:h-14 text-lego-blue animate-spin" />
          <p className="text-sm sm:text-lg text-gray-300">Detecting pieces...</p>
        </div>
      )}
      ...
      {uiState === "previewing" && previewData && (
        <div className="space-y-3">
          {/* Header bar */}
          <div className="flex flex-wrap items-center gap-2 bg-gray-800/60 border border-gray-700 rounded-lg px-3 sm:px-5 py-2 sm:py-3">
            <div className="flex items-center gap-1.5 flex-1 min-w-0">
              <ScanSearch className="w-4 h-4 sm:w-5 sm:h-5 text-purple-400 shrink-0" />
            ...
            <span className="text-xs sm:text-sm text-gray-500">Review before identifying</span>
          </div>
        ...
        <div className="space-y-4">
          <div className="flex flex-col items-center gap-3 py-8 sm:py-16">
            <div className="relative">
              <ScanSearch className="w-10 h-10 sm:w-16 sm:h-16 text-lego-yellow" />
              <Loader2 className="w-4 h-4 sm:w-6 sm:h-6 text-lego-yellow animate-spin absolute -bottom-1 -right-1" />
            </div>
      ...
      {uiState === "failed" && (
        <div className="space-y-3">
          <div className="flex flex-col items-center gap-3 py-8 sm:py-16">
            <X className="w-8 h-8 sm:w-14 sm:h-14 text-lego-red" />
            <p className="text-sm sm:text-lg text-gray-300">Scan failed</p>
            <p className="text-xs sm:text-sm text-gray-500 text-center">{scan?.errorMessage || "Something went wrong. Please try again."}</p>
          </div>
          <Button
            variant="outline"
            className="w-full"
        ...
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap items-center gap-2 bg-gray-800/60 border border-gray-700 rounded-lg px-3 sm:px-5 py-2 sm:py-3">
            <div className="flex items-center gap-1.5 flex-1 min-w-0">
              ...
              <span className="text-sm sm:text-base font-medium text-green-300">Scan complete</span>
            </div>
            <div className="flex gap-3 text-xs sm:text-sm font-mono text-gray-400 flex-wrap">
              ...
              {groupedResults.map((grp, gi) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div
                      className="flex gap-2.5 sm:gap-4 px-2.5 sm:px-4 py-2 sm:py-3 cursor-pointer hover-elevate"
                    ...
                    >
                      ...
                      {(() => {
                        ...
                        return (
                          <div
                            className={`flex-shrink-0 w-12 h-12 sm:w-16 sm:h-16 rounded bg-gray-800/80 flex items-center justify-center overflow-hidden ${lightboxSrc ? 'cursor-pointer hover-elevate' : ''}`}
                      ...
                      <div className="flex-1 min-w-0 flex flex-col gap-0.5 justify-center">
                        ...
                        <div className="flex items-center gap-1">
                          <p className="text-xs sm:text-sm font-semibold text-white leading-tight flex-1">
                            {grp.partName || "Unknown Part"}
                          </p>
                          {grp.itemType === 'MINIFIG' && (
                            <span className="text-[9px] sm:text-[11px] font-semibold uppercase tracking-wider text-amber-400 bg-amber-900/40 border border-amber-500/30 rounded px-1 py-0.5 flex-shrink-0">Fig</span>
                          )}
                          {grp.partNo && (
                            <a
                          ...
                          <ChevronRight
                            className={`w-3.5 h-3.5 sm:w-4 sm:h-4 text-gray-500 flex-shrink-0 transition-transform duration-150 ${isExpanded ? "rotate-90" : ""}`}
                          />
                        </div>

                        {/* Part no · qty · confidence · crop count */}
                        <div className="flex items-center gap-1.5 flex-wrap">
                          {grp.partNo && <span className="font-mono text-[10px] sm:text-xs text-gray-300">{grp.partNo}</span>}
                          {stockLabel && (
                            <span className="text-[10px] sm:text-xs text-green-400 font-medium">· {stockLabel}</span>
                          )}
                          <span className={`text-[10px] sm:text-xs font-medium capitalize ${confidenceColor(bestConfidence)}`}>
                            · {bestConfidence}
                          </span>
                          {grp.entries.length > 1 && (
                            <span className="text-[10px] sm:text-xs text-gray-500">· {grp.entries.length} crops</span>
                          )}
                        </div>
                      </div>
                    </div>
                    ...
                    {isExpanded && (
                      <div className="border-t border-purple-500/10 px-2 sm:px-4 py-1.5 sm:py-2.5 space-y-1 sm:space-y-2">
                        {/* Column headers */}
                        <div className="flex items-center gap-1 px-2 pb-0.5">
                          <div className="flex-1 min-w-0">
                            <span className="text-[9px] sm:text-[11px] uppercase tracking-wider text-gray-500">{grp.itemType === 'MINIFIG' ? 'Minifigure' : 'Color'}</span>
                          </div>
                          <span className="text-[9px] sm:text-[11px] uppercase tracking-wider text-gray-500 w-14 sm:w-20 text-right flex-shrink-0">N Cur</span>
                          <span className="text-[9px] sm:text-[11px] uppercase tracking-wider text-gray-500 w-[58px] sm:w-[72px] text-right flex-shrink-0">N Score</span>
                          <span className="text-[9px] sm:text-[11px] uppercase tracking-wider text-gray-500 w-14 sm:w-20 text-right flex-shrink-0">U Cur</span>
                          <span className="text-[9px] sm:text-[11px] uppercase tracking-wider text-gray-500 w-[58px] sm:w-[72px] text-right flex-shrink-0">U Score</span>
                        </div>
                        ...
                        {detectedColorEntries.map((entry, ei) => {
                          ...
                          return (
                            <div key={ei} className="rounded-lg border border-purple-500/40 bg-purple-900/25 px-2 sm:px-3 py-1.5 sm:py-2">
                              <div className="flex items-center gap-1 mb-1">
                                <Sparkles className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-purple-400 flex-shrink-0" />
                                <span className="text-[9px] sm:text-[11px] uppercase tracking-wider text-purple-400 font-semibold">Best Match</span>
                                <span className={`ml-1 text-[9px] sm:text-[11px] font-medium capitalize ${confidenceColor(entry.confidence)}`}>
                                  · {entry.confidence} confidence
                                </span>
                                {entry.cropIndex != null && (
                                  <span className="ml-auto text-[9px] sm:text-[11px] font-mono text-gray-500 flex-shrink-0">crop #{entry.cropIndex + 1}</span>
                                )}
                              </div>
                              <div className="flex items-center gap-1 min-w-0">
                                <div className="flex flex-col flex-1 min-w-0">
                                  <div className="flex items-center gap-1 min-w-0">
                                    {inStock && <Check className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-emerald-400 flex-shrink-0" />}
                                    {inStock && <span className="text-[10px] sm:text-xs font-mono text-gray-200 flex-shrink-0">×{entry.ourQtyNew + entry.ourQtyUsed}</span>}
                                    {grp.itemType !== 'MINIFIG' && (entry.colorRgb ? (
                                      <span className="w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full flex-shrink-0 border border-gray-500" style={{ backgroundColor: `#${entry.colorRgb}` }} />
                                    ) : (
                                      <span className="w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full flex-shrink-0 bg-gray-600" />
                                    ))}
                                  ...
                                  {peak ? (
                                    <span className="text-[9px] sm:text-[11px] pl-0.5 text-purple-400">peak ${peak.toFixed(2)}</span>
                                  ) : null}
                                </div>
                                <span className="text-[10px] sm:text-xs font-mono text-gray-200 w-14 sm:w-20 text-right flex-shrink-0">
                                  {entry.ourPriceNew != null ? `$${entry.ourPriceNew.toFixed(2)}` : '—'}
                                </span>
                                <span className={`text-[10px] sm:text-xs font-mono font-bold w-[58px] sm:w-[72px] text-right flex-shrink-0 ${entryScoreColor(nScore)}`}>
                                  {nScore != null ? `${nScore}×` : '—'}
                                </span>
                                <span className="text-[10px] sm:text-xs font-mono text-gray-200 w-14 sm:w-20 text-right flex-shrink-0">
                                  {entry.ourPriceUsed != null ? `$${entry.ourPriceUsed.toFixed(2)}` : '—'}
                                </span>
                                <span className={`text-[10px] sm:text-xs font-mono font-bold w-[58px] sm:w-[72px] text-right flex-shrink-0 ${entryScoreColor(uScore)}`}>
                                  {uScore != null ? `${uScore}×` : '—'}
                                </span>
                              </div>
                            </div>
                        ...
                        <div className="space-y-0.5 sm:space-y-1">
                          <div className="flex items-center gap-1 px-2 pb-0.5 pt-1.5">
                            <div className="flex-1 min-w-0">
                              <span className="text-[9px] sm:text-[11px] uppercase tracking-wider text-gray-500">All known color variants</span>
                            </div>
                            <span className="text-[9px] sm:text-[11px] uppercase tracking-wider text-gray-500 w-14 sm:w-20 text-right flex-shrink-0">N Cur</span>
                            <span className="text-[9px] sm:text-[11px] uppercase tracking-wider text-gray-500 w-[58px] sm:w-[72px] text-right flex-shrink-0">N Score</span>
                            <span className="text-[9px] sm:text-[11px] uppercase tracking-wider text-gray-500 w-14 sm:w-20 text-right flex-shrink-0">U Cur</span>
                          ...
                          {otherLots.map((lot, li) => {
                            ...
                            return (
                              <div
                                key={li}
                                className="bg-gray-900/50 border border-gray-700/60 rounded-lg px-2 sm:px-3 py-1.5 sm:py-2"
                                ...
                                <div className="flex items-center gap-1.5 min-w-0">
                                  {lot.imageUrl ? (
                                    <img
                                      src={lot.imageUrl}
                                      alt={lot.colorName || ''}
                                      className="w-6 h-6 sm:w-8 sm:h-8 object-contain rounded flex-shrink-0 bg-gray-800"
                                      onError={e => { (e.target as HTMLImageElement).style.display = 'none'; }}
                                    />
                                  ) : (
                                    <div className="w-6 h-6 sm:w-8 sm:h-8 flex-shrink-0 rounded bg-gray-800" />
                                  )}
                                  <div className="flex flex-col flex-1 min-w-0">
                                    <div className="flex items-center gap-1 min-w-0">
                                      {lotInStock && <Check className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-emerald-400 flex-shrink-0" />}
                                      {lotInStock && <span className="text-[10px] sm:text-xs font-mono text-gray-200 flex-shrink-0">×{lot.qtyNew + lot.qtyUsed}</span>}
                                      {lot.colorRgb ? (
                                        <span className="w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full flex-shrink-0 border border-gray-500" style={{ backgroundColor: `#${lot.colorRgb}` }} />
                                      ) : (
                                        <span className="w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full flex-shrink-0 bg-gray-600" />
                                      )}
                                      <span className={`text-[10px] sm:text-xs truncate ${lotInStock ? 'text-gray-100' : 'text-gray-400'}`}>{lot.colorName || '—'}</span>
                                    </div>
                                    {lotPeak && (
                                      <span className="text-[9px] sm:text-[11px] pl-0.5 text-purple-400">peak ${lotPeak.toFixed(2)}</span>
                                    )}
                                  </div>
                                  <span className="text-[10px] sm:text-xs font-mono text-gray-200 w-14 sm:w-20 text-right flex-shrink-0">
                                    {lot.priceNew != null ? `$${lot.priceNew.toFixed(2)}` : '—'}
                                  </span>
                                  <span className={`text-[10px] sm:text-xs font-mono font-bold w-[58px] sm:w-[72px] text-right flex-shrink-0 ${entryScoreColor(lotNScore)}`}>
                                    {lotNScore != null ? `${lotNScore}×` : '—'}
                                  </span>
                                  <span className="text-[10px] sm:text-xs font-mono text-gray-200 w-14 sm:w-20 text-right flex-shrink-0">
                                    {lot.priceUsed != null ? `$${lot.priceUsed.toFixed(2)}` : '—'}
                                  </span>
                                  <span className={`text-[10px] sm:text-xs font-mono font-bold w-[58px] sm:w-[72px] text-right flex-shrink-0 ${entryScoreColor(lotUScore)}`}>
