...
export default function OrdersDashboard({ onItemClick, activeDrawer, onDrawerChange }: OrdersDashboardProps) {
  ...
  return (
    <div className="p-2 space-y-1.5 bg-gradient-to-br from-lego-orange/5 to-transparent rounded-lg border border-lego-orange/10 shadow-[0_0_15px_rgba(251,146,60,0.1)]">
      ...
      <div className="bg-gray-900/50 border border-orange-500/20 rounded-lg p-4 md:p-5 lg:p-6" data-testid="section-pending-orders">
        <div className="flex items-center gap-1.5 md:gap-2 lg:gap-2.5 mb-2 md:mb-3 lg:mb-4">
          ...
          <h3 className="text-sm md:text-base lg:text-lg font-semibold text-orange-400 uppercase tracking-wide">Action Items - Pending Orders</h3>
        </div>
            ...
            pendingOrders.map((order) => (
              ...
              >
                <div className="flex items-center gap-2 flex-1 min-w-0">
                  <ShoppingCart className="w-4 h-4 md:w-5 md:h-5 lg:w-6 lg:h-6 text-orange-400 flex-shrink-0" />
                  <span className="text-gray-200 font-mono text-sm md:text-base lg:text-lg font-medium">#{order.orderNumber}</span>
      ...
      <div className="bg-gray-900/50 border border-blue-500/20 rounded-lg p-4 md:p-5 lg:p-6" data-testid="section-recent-shipments">
        <div className="flex items-center gap-1.5 md:gap-2 lg:gap-2.5 mb-2 md:mb-3 lg:mb-4">
          <Package className="w-4 h-4 md:w-5 md:h-5 lg:w-6 lg:h-6 text-blue-400" />
          <h3 className="text-sm md:text-base lg:text-lg font-semibold text-blue-400 uppercase tracking-wide">Recent Activity - Shipped Orders</h3>
        </div>
        <div className="space-y-1.5">
          {recentShipments.length > 0 ? (
            ...
            <div className="text-xs md:text-sm lg:text-base text-gray-500 italic">No recent shipments</div>
          )}
        </div>
      </div>

      {/* Highlights - High Value Orders */}
      <div className="bg-gray-900/50 border border-green-500/20 rounded-lg p-4 md:p-5 lg:p-6" data-testid="section-high-value-orders">
        <div className="flex items-center gap-1.5 md:gap-2 lg:gap-2.5 mb-2 md:mb-3 lg:mb-4">
          <TrendingUp className="w-4 h-4 md:w-5 md:h-5 lg:w-6 lg:h-6 text-green-400" />
          <h3 className="text-sm md:text-base lg:text-lg font-semibold text-green-400 uppercase tracking-wide">Highlights - Top Value Orders</h3>
        </div>
        <div className="space-y-1.5">
          {highValueOrders.length > 0 ? (
            highValueOrders.map((order) => (
              ...
              >
                <div className="flex items-center gap-2 flex-1 min-w-0">
                  <TrendingUp className="w-4 h-4 md:w-5 md:h-5 lg:w-6 lg:h-6 text-green-400 flex-shrink-0" />
                  <span className="text-gray-200 font-mono text-sm md:text-base lg:text-lg font-medium">#{order.orderNumber}</span>
                  <span className="text-gray-400 text-xs md:text-sm lg:text-base">{order.customerUsername}</span>
                </div>
                <span className="text-lego-green font-mono font-medium text-sm md:text-base lg:text-lg ml-2 flex-shrink-0">${Number(order.orderTotal).toFixed(2)}</span>
              </div>
            ))
          ) : (
            <div className="text-xs md:text-sm lg:text-base text-gray-500 italic">No orders to display</div>
          )}
        </div>
      </div>
