...
export default function OrdersDashboard({ onItemClick, activeDrawer, onDrawerChange }: OrdersDashboardProps) {
  ...
  return (
    <div className="p-2 space-y-1.5 bg-gradient-to-br from-lego-orange/5 to-transparent rounded-lg border border-lego-orange/10 shadow-[0_0_15px_rgba(251,146,60,0.1)]">
      ...
      <div className="bg-gray-900/50 border border-orange-500/20 rounded-lg p-3 md:p-4 lg:p-6" data-testid="section-pending-orders">
        <div className="flex items-center gap-1.5 md:gap-2 lg:gap-2.5 mb-2 md:mb-3 lg:mb-4">
          <AlertCircle className="w-3.5 h-3.5 md:w-4 md:h-4 lg:w-5 lg:h-5 text-orange-400" />
          <h3 className="text-xs md:text-sm lg:text-base font-semibold text-orange-400 uppercase tracking-wide">Action Items - Pending Orders</h3>
        </div>
            ...
            pendingOrders.map((order) => (
              ...
              >
                <div className="flex items-center gap-2 flex-1 min-w-0">
                  <ShoppingCart className="w-3.5 h-3.5 md:w-4 md:h-4 lg:w-5 lg:h-5 text-orange-400 flex-shrink-0" />
                  <span className="text-gray-200 font-mono text-xs md:text-sm lg:text-base font-medium">#{order.orderNumber}</span>
      ...
      <div className="bg-gray-900/50 border border-blue-500/20 rounded-lg p-3 md:p-4 lg:p-6" data-testid="section-recent-shipments">
        <div className="flex items-center gap-1.5 md:gap-2 lg:gap-2.5 mb-2 md:mb-3 lg:mb-4">
          <Package className="w-3.5 h-3.5 md:w-4 md:h-4 lg:w-5 lg:h-5 text-blue-400" />
          <h3 className="text-xs md:text-sm lg:text-base font-semibold text-blue-400 uppercase tracking-wide">Recent Activity - Shipped Orders</h3>
        </div>
        <div className="space-y-1.5">
          {recentShipments.length > 0 ? (
            recentShipments.map((order) => (
              ...
              >
                <div className="flex items-center gap-2 flex-1 min-w-0">
                  <div className="w-2 h-2 md:w-2.5 md:h-2.5 lg:w-3 lg:h-3 rounded-full bg-green-400 flex-shrink-0" />
                  <span className="text-gray-200 font-mono text-xs md:text-sm lg:text-base font-medium">#{order.orderNumber}</span>
                  <span className="text-gray-400 text-[11px] md:text-xs lg:text-sm">{order.customerUsername}</span>
                </div>
                <span className="text-lego-green font-mono font-medium text-xs md:text-sm lg:text-base ml-2 flex-shrink-0">${Number(order.orderTotal || 0).toFixed(2)}</span>
            ...
            <div className="text-[11px] md:text-xs lg:text-sm text-gray-500 italic">No recent shipments</div>
          )}
        </div>
      </div>

      {/* Highlights - High Value Orders */}
      <div className="bg-gray-900/50 border border-green-500/20 rounded-lg p-3 md:p-4 lg:p-6" data-testid="section-high-value-orders">
        <div className="flex items-center gap-1.5 md:gap-2 lg:gap-2.5 mb-2 md:mb-3 lg:mb-4">
          <TrendingUp className="w-3.5 h-3.5 md:w-4 md:h-4 lg:w-5 lg:h-5 text-green-400" />
          <h3 className="text-xs md:text-sm lg:text-base font-semibold text-green-400 uppercase tracking-wide">Highlights - Top Value Orders</h3>
        </div>
        <div className="space-y-1.5">
          {highValueOrders.length > 0 ? (
            highValueOrders.map((order) => (
              ...
              >
                <div className="flex items-center gap-2 flex-1 min-w-0">
                  <TrendingUp className="w-3.5 h-3.5 md:w-4 md:h-4 lg:w-5 lg:h-5 text-green-400 flex-shrink-0" />
                  <span className="text-gray-200 font-mono text-xs md:text-sm lg:text-base font-medium">#{order.orderNumber}</span>
                  <span className="text-gray-400 text-[11px] md:text-xs lg:text-sm">{order.customerUsername}</span>
                </div>
                <span className="text-lego-green font-mono font-medium text-xs md:text-sm lg:text-base ml-2 flex-shrink-0">${Number(order.orderTotal).toFixed(2)}</span>
              </div>
            ))
          ) : (
            <div className="text-[11px] md:text-xs lg:text-sm text-gray-500 italic">No orders to display</div>
          )}
        </div>
      </div>
