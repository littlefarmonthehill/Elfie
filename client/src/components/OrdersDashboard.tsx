...
export default function OrdersDashboard({ onItemClick, activeDrawer, onDrawerChange, onSalesDrawer, dateRange: initialDateRange = 'mtd', onOpenSettings, desktopMode, tvSplit, compact, initialPanelTab }: OrdersDashboardProps) {
  ...
  return (
      ...
      <SearchDrawer
        ...
        onClose={() => { setOrderSearchOpen(false); setOrderSearchInput(''); }}
        title="Search Orders"
      ...
      >
        {orderSearchInput.trim().length < 2 ? (
          ...
          <div className="rounded-lg border border-orange-500/20 bg-gray-900/60 divide-y divide-gray-800/60 overflow-hidden">
            {orderSearchResults.map(o => (
              <button
                ...
                onClick={() => {
                  ...
                  setOrderSearchInput('');
                  if (onItemClick) onItemClick('order', o.id);
                }}
