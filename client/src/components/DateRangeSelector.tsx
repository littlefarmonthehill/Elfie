...
export default function DateRangeSelector({ value, onChange, className = "" }: DateRangeSelectorProps) {
  ...
  return (
    <div className={`flex items-center gap-1.5 md:gap-4 lg:gap-5 ${className}`}>
      <div className="flex items-center gap-1 md:gap-2.5 lg:gap-3 flex-shrink-0">
      ...
      <div className="flex gap-1 md:gap-3 lg:gap-4 overflow-x-auto scrollbar-hide">
        {options.map((option) => (
          <button
            key={option.value}
            onClick={() => onChange(option.value)}
            className={`text-[10px] md:text-base lg:text-lg font-bold py-1 md:py-2 lg:py-2.5 px-1.5 md:px-4 lg:px-5 rounded transition-all whitespace-nowrap flex-shrink-0 ${
              value === option.value
